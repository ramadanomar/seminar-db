...
FROM angajati
WHERE data_angajare >= date '1994-01-01'
    AND data_angajare < date '1995-01-01';
--
...
FROM angajati
WHERE salariul > a;
LOOP FETCH c_angajat INTO v_nume,
v_salariul;
EXIT
WHEN c_angajat %NOTFOUND;
DBMS_OUTPUT.PUT_LINE(
    'Nume: ' || v_nume || ', Salariul: ' || v_salariul
);
END LOOP;
CLOSE c_angajat;
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('404');
WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('Too many lines returned');
...
BEGIN v_sql_stmt := 'SELECT id_angajat, nume, salariul, id_functie FROM angajati WHERE id_manager = :manager ORDER BY ' || v_order_column;
OPEN c_employee FOR v_sql_stmt USING v_manager_id;
LOOP FETCH c_employee INTO v_employee;
EXIT
WHEN c_employee %NOTFOUND;
DBMS_OUTPUT.PUT_LINE(
    'ID: ' || v_employee.id_angajat || ', Name: ' || v_employee.nume || ', Salary: ' || v_employee.salariul || ', Function ID: ' || v_employee.id_functie
);
END LOOP;
CLOSE c_employee;
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('No employees found for this manager.');
WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('An error occurred: ' || SQLERRM);
END;
--
SELECT nume
...
FROM angajati
WHERE salariul NOT IN ('2500', '3500', '7000')
    AND id_functie IN ('SA_REP', 'ST_CLERK');
--
SELECT nume, salariul, comision
FROM angajati
WHERE comision IS NOT NULL
